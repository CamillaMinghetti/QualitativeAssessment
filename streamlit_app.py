...
# ----- Pagina 2: Spiegazione del Questionario -----
    ...
    st.image("path_to_your_image.jpg", caption="Example Depth Maps from AI Models", use_column_width=True)
...
# ----- Pagina 4: Questionario -----
    ...
    if clinician == "Yes":
        experience_level = st.radio("What is your experience level?", ["Specialist", "Resident", "Expert"])
    ...
    name = st.text_input("Please enter your name")
        ...
        if name:
            ...
            st.subheader(f"Domanda {question_index + 1}")
            ...
            response = st.radio(
                "Quale delle due mappe rappresenta meglio la profondità?",
                options,
            ...
            with col1:
                if st.button("Indietro") and question_index > 0:
                    st.session_state["question_index"] -= 1
                    st.rerun()
            with col2:
                if st.button("Avanti", disabled=(st.session_state["responses"][question_index] is None)) and question_index < len(video_paths) - 1:
                    st.session_state["question_index"] += 1
                    st.rerun()

            # Submission Block
            if question_index == len(video_paths) - 1 and st.button("Invia le Risposte", disabled=(st.session_state["responses"][question_index] is None)):
                ...
                new_data = {
                    "Nome": name,
                    "Medico": clinician,
                    "Esperienza": experience_level if clinician == "Yes" else None,
                    "Procedure Eseguite": procedures_performed if clinician == "Yes" else None,
                }
                for i in range(len(video_paths)):
                    new_data[f"Domanda {i+1}"] = st.session_state["responses"][i] if st.session_state["responses"][i] else "No Response"

                try:
                    with open(file_path, "w") as f:
                        json.dump(new_data, f, indent=4)
                except Exception as e:
                    st.error(f"Errore nel salvataggio del file JSON: {e}")
                ...
                st.success("Le tue risposte sono state salvate!")
                st.stop()
